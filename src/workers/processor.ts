...
import { jobs, deliveries, pipelines, subscribers } from '../database/schema';
...
export const processJob = async (jobId: number, pipelineId: number, payload: any) => {
  ...
  try {
    ...
    for (const subscriber of pipeline.subscribers) {
      ...
      await deliverToSubscriber(delivery.id, jobId, subscriber.url, processedPayload);
    }
